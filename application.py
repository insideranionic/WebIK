...
def quiz():
    quiz_name = request.form.get('dropdown')
    # hoe komen we aan de naam van de quiz bij name= "naam"????
    catte= db.execute("SELECT category FROM new_quizes WHERE name = :name", name=quiz_name)
    diffi = db.execute("SELECT difficulty FROM new_quizes WHERE name = :name", name=quiz_name)
    aantal_vragen= db.execute("SELECT amount_of_questions FROM new_quizes WHERE name = :name", name=quiz_name)
    Type= db.execute("SELECT type FROM new_quizes WHERE name = :name", name=quiz_name)
